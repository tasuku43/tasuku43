...
2019年にソフトウェアエンジニアとしてキャリアをスタートし、PHP部でのバックエンド開発から始め、段階的に技術領域を拡大してきました。PHPでは静的解析ツールの導入を主導し、11年目のコードベースの品質向上に貢献。その後、TypeScriptとReact/Next.jsを用いたフロントエンド開発にも携わり、設計からデプロイまでの一貫した開発フローを習得しました。さらに、フルサイクル開発者としてAWS（CloudFront、API Gateway、CloudWatch、IAMなど）を活用したインフラ運用や、DataDogを用いた監視体制の構築も経験。Kubernetesのマニフェスト修正など、クラウドネイティブな技術領域にも対応しています。

現在は認証チームに所属し、PdMやデザイナーを含むチームメンバーと密に協力しながら、認証ドメインに特化したシステムの要件定義から設計、実装、運用までを担当。チーム全体の開発プロセスを可視化するバリューストリームマッピングを主導し、プロセス改善にも力を入れています。
...
#### SREグループ(2025年4月 ~ )
- バリューストリーム全体を俯瞰し、継続的な改善活動を推進できる人材を目指すキャリア戦略の一環として、SREグループへ異動。
- 今後は、インフラ領域の専門性を高めながら、開発と運用を接続する改善活動を横断的に支援していく予定。
