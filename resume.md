...
- 主な実績
    ...
    - Dockerfileをリポジトリ分離し、QEMUエミュレーションから**各アーキテクチャ専用ランナーによる並列ビルド＋manifest統合**へ刷新。GitHub Actionsでマルチアーキテクチャ対応パイプラインを構築し、コンテナイメージのビルド時間を**約50%短縮**
...
- AWS（5年）：日常的に使用していたサービスは以下の通りです。
    - Lambda, ECR, DynamoDB, Billing and Cost Management, CloudFront, SQS, S3, Athena
- Kubernetes（2年）：基礎的な理解と操作が可能です。具体的なスキルレベルは以下の通りです
...
- CircleCI
- GitHub Actions（テスト実行や静的解析などのワークフローを新規構築。CircleCIからの移行も担当し、既存の実行内容を正確に把握した上でGitHub Actionsに最適な形で置き換えを実施）

### 監視

- DataDog（ダッシュボード・モニターの整備を通じて、システム状態の可視化と異常検知体制を構築）
